module CmsinoHelper
  ...
  def editable_content(name)
    ...
    content = Cmsino::Content.find_or_create_by_page_and_name(@cmsino_page.name, name)
...
#{content.text}#{editable_content_link(content)}
  ...
  def editable_image_content(name)
    ...
    content = Cmsino::ImageContent.find_or_create_by_page_and_name(@cmsino_page.name, name)
